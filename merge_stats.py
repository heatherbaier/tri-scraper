...
if __name__ == "__main__":
    ...
    df.columns = ["Event", "Date", 'Time', "Score", "FN", "LN"]
    ...
    df["Race"] = np.where(df.Event.str.contains("Ironman"), "Ironman", df.Race)
    ...
    # df.to_csv("./participant_stats_raw.csv", index = False)
    ...
    recent_olympic["Name"] = recent_olympic["FN"] + " " + recent_olympic["LN"]
    # recent_olympic = pd.DataFrame(recent_olympic.groupby(["Name"])["Score"].mean()).reset_index()
    recent_olympic['Time'] = pd.to_timedelta(recent_olympic.Time)#, format='%H:%M:%S')#.dt.time
    ...
    recent_olympic["Time"] = recent_olympic["Time"].astype(str).str.split(" ").str[2].str.split(".").str[0]
    ...
    # filter for age, gender and race length
